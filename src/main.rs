...
async fn run_demo_cycle(cfg: &AppCfg, ws: &mut WsClient) -> Result<()> {
    info!("");
    info!("🎮 ════════════════════════════════════════");
    info!("🎮 DEMO MODE: Mencari 3 kekalahan beruntun");
    info!("🎮 ════════════════════════════════════════");
    ...
    loop {
        trade_count += 1;
        info!("");
        info!("🎯 DEMO Trade #{} | Streak: {}/3 losses", trade_count, losses_in_a_row);
        ...
        if !won {
            losses_in_a_row += 1;
            info!("📉 Loss recorded | Streak: {}/3", losses_in_a_row);
            
            if losses_in_a_row >= 3 {
                info!("");
                info!("✨ ════════════════════════════════════════");
                info!("✨ TARGET REACHED: 3 losses in a row!");
                info!("✨ Switching to REAL MODE...");
                info!("✨ ════════════════════════════════════════");
                break;
            }
        } else {
            warn!("⚠️  WIN detected → Reset streak to 0/3");
            losses_in_a_row = 0;
        }
...
async fn main() -> Result<()> {
    ...
    info!("🤖 Strategy: 3 Demo Losses → Real Mode (2-step Martingale)");
    info!("🤖 ════════════════════════════════════════════════════════");
